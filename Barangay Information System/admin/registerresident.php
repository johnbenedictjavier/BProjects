...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($isUpdate) {
        ...
        $sqlRes = "INSERT INTO residents (first_name, middle_name, last_name, present_address, address_type, provincial_address, gender, civil_status, birthdate, birthplace, height, weight, contact_number, email_address, religion, occupation, house_number, household_id, status, created_at) 
                   VALUES ('$fname', '$mname', '$lname', '$addr', '$addr_t', '$p_addr', '$gen', '$civ', '$bdate', '$bplace', '$h', '$w', '$cont', '$email', '$rel', '$occ_main', '$hnum', '$hh_id', 'Registered', NOW())";
        $conn->query($sqlRes);
    ...
    if (!empty($_POST['occ_name'])) {
        foreach ($_POST['occ_name'] as $key => $val) {
            if (!empty($val)) {
                ...
                $o_gen = mysqli_real_escape_string($conn, $_POST['occ_gen'][$key]);
                ...
                $o_cnbr = mysqli_real_escape_string($conn, $_POST['occ_cnbr'][$key]);
                $conn->query("INSERT INTO resident_occupants (resident_id, full_name, position_in_family, gender, age, birthdate, contact_number, civil_status, occupation) VALUES ('$resident_id', '$o_name', '$o_pos','$o_gen', '$o_age', '$o_bdate', '$o_cnbr', '$o_civ', '$o_occ')");
            }
        }
    }

    echo "<script>window.location='residents.php?new_id=$resident_id';</script>";
}
?>
    ...
    <div class="row justify-content-center">
        <div class="col-lg-11">
            <div class="d-flex justify-content-between align-items-center mb-4">
                ...
                <a href="residents.php" class="btn btn-outline-secondary btn-sm">Cancel</a>
            </div>
            ...
            <div class="card border-0 shadow-sm p-4">
                ...
                <form id="censusForm" method="POST">
                    <div class="step-container active" id="step1">
                        ...
                        <div class="row g-3">
                            <div class="col-md-4"><label>First Name</label><input type="text" name="first_name" class="form-control" value="<?= $res['first_name']??'' ?>" required></div>
                            <div class="col-md-4"><label>Middle Name</label><input type="text" name="middle_name" class="form-control" value="<?= $res['middle_name']??'' ?>"></div>
                            <div class="col-md-4"><label>Last Name</label><input type="text" name="last_name" class="form-control" value="<?= $res['last_name']??'' ?>" required></div>
                            <div class="col-md-3">
                                <label>Gender</label>
                        ...
                        <div class="table-responsive-wide">
                            <table class="table table-bordered" id="occupantTable">
                                <thead class="table-light"><tr>
                                    <th style="width: 33%;">Full Name (LName, FName MName)</th>
                    ...
                    <th style="width: 13%;">Contact Number</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <select name="occ_gen[]" class="form-select border-0">
                                        ...
                                        <td><input type="text" name="occ_cnbr[]" class="form-control border-0" value="<?= $o['contact_number'] ?>"></td>
                                    </tr>
                                    <?php endforeach; else: ?>
                                    <tr>
                                        ...
                                        <td><input type="text" name="occ_cnbr[]" class="form-control border-0"></td>
                                    </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
